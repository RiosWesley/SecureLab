...
- **Funcionalidades**:
  ...
  - Filtros de busca por data, usuário e porta
  - Exibição limitada de eventos do sistema
...
| Logs | ⚠️ Parcialmente implementado | Interface básica implementada, funcionalidade limitada |
...
### Implementação de Novas Páginas e Atualização do Escopo (Março 2025)

Foram implementadas novas páginas no sistema e atualizado o escopo do projeto:
...
3. **Página de Logs (Parcial)**:
   - Implementação básica da interface de visualização de logs
   - Filtros e opções de pesquisa
   - Funcionalidade de exibição limitada

4. **Redução de Escopo**:
   - Removidos os módulos de Grupos e Alertas do escopo do projeto
   - Foco direcionado para as funcionalidades principais do sistema

As novas páginas foram implementadas principalmente no frontend, com funcionalidade backend limitada.
